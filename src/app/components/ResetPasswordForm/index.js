...
const ResetPasswordForm = function ResetPasswordForm({ handleSubmit, submitting }) {
  return (
    <form className={styles.login__form} onSubmit={handleSubmit}>
      <Field name="password" id="password" type="password" component={RenderField} label="New Password" />
      <Field name="confirmPassword" id="confirmPassword" type="password" component={RenderField} label="Confirm Password" />
      <div>
        <button className="btn btn-primary btn-block" type="submit" disabled={submitting}>Update Password</button>
      </div>
    </form>
  );
};
  ...
  return {
    ...
    onSubmit(formData) {
      ...
      return new Promise((resolve, reject) => {
        dispatch(resetPasswordRequest({ data, resolve, reject }));
      });
    },
  };
}
